...
def articles():
    ...
    # Setting geo as entered location from HTML form
    geo = request.args.get("geo")

    # Checking if input was entered
    if not geo:
        raise RuntimeError("No location given")

    # Fetching articles and assigning it to a variable
    articles = lookup(geo)

    # Returning a maximum of 5 articles
    if len(articles) > 5:
...
def search():
    ...
    # Actively fetching query entered into HTML form
    q = request.args.get("q") + "%"

    # Selecting all places that contain entered query
    place = db.execute("SELECT * FROM places WHERE postal_code \
                          LIKE :q OR place_name LIKE :q OR admin_name1 LIKE :q", q=q)

    # Returning upto 10 places that match entered query
    if len(place) > 10:
